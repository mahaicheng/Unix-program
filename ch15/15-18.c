...
int main() {
  ...
  if ((pid = fork()) < 0)
    ...
    while (fgets(line, MAXLINE, stdin) != NULL) {
      ...
      if (write(fd1[1], line, n) != n) err_sys("write error");
      if ((n = read(fd2[0], line, MAXLINE)) < 0) err_sys("read error");
      if (n == 0) {
    ...
    if (ferror(stdin)) err_sys("fgets error");
    ...
    if (fd2[1] != STDOUT_FILENO) {
      if (dup2(fd2[1], STDOUT_FILENO) != STDOUT_FILENO) err_sys("dup2 error");
      close(fd2[1]);
    }
